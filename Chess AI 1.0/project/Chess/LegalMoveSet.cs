...
namespace Chess
{
    ...
    public class LegalMoveSet
    {
        ...
        public static bool IsTerminal(ChessBoard b)
        {
            if (!hasMoves(b, Player.BLACK) && isCheck(b, Player.BLACK)) return true;
            if (!hasMoves(b, Player.WHITE) && isCheck(b, Player.WHITE)) return true;
            return false;
        }
